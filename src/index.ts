import { AIService } from './application/ai/index.js';
import { OpenAIClient } from './infrastructure/ai/index.js';
import { logger } from './infrastructure/logger/index.js';

const aiService = new AIService(new OpenAIClient());

try {
  logger.info({ event: 'application.started' }, 'Starting application');

  const response = await aiService.generateResponse('Tell me a joke about programming.');
